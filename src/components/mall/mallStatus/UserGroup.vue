        ...
        <div class="sectorSales_main">
            ...
            <div class="content">
                <div class="content_header">
                    <el-radio-group v-model="contentHeader" @change="getSalesByCity">
                    ...
                    <div class="content_time">
                        <div class="date_item">
                            <el-select v-model="dateTypeSelect" placeholder="请选择" size="mini" @change="getSalesByCity">
                                <el-option
    ...
    export default {
                ...
                breadData: [
                    {
                        ...
                        id: 3,
                        name: '用户群体数据',
                        urls: '/index/mall/mallStatus/UserGroup',
                        icon: 'icon-home'
                    }
                ],
        ...
        methods: {
                ...
                if(this.dateTypeSelect==='week'){
                    ...
                    dateList.map(item=>{
                        this.typeList.map(type=>{
                            ...
                            this.saleData.map(tmp=>{
                                if(tmp.category_id===type.menu_id && item===tmp.created_at.split(' ')[0]){
                                    type.total+=tmp.num;
                                }
                            });
                    ...
                    dates.map(item=>{
                        ...
                        this.typeList.map(type=>{
                            type.total=0;
                            this.saleData.map(tmp=>{
                                if(tmp.category_id===type.menu_id && yearMonth+'-'+item===tmp.created_at.split(' ')[0]){
                                    type.total+=tmp.num;
                                }
                            });
                            type.data.push(type.total);
                    ...
                    dates.map(item=>{
                        ...
                        this.typeList.map(type=>{
                            type.total=0;
                            this.saleData.map(tmp=>{
                                if(tmp.category_id===type.menu_id && date===moment(tmp.created_at).format('YYYY-MM')){
                                    type.total+=tmp.num;
                                }
                            });
                            type.data.push(type.total);
            ...
            async getProvince(){
                ...
                await this.HttpClient.post('/admin/address/orderDistributionMap',parameters)
                    .then(res=>{
                        console.log(res);
                        if(res.data.code===200){
            ...
            getSalesByCity(){
                ...
                this.HttpClient.post('/admin/address/orderCity',parameters)
                    .then(res=>{
                        console.log(res);
                        if(res.data.code===200){
...
<style lang="less">
    .sectorSales {
        ...
        .sectorSales_main::-webkit-scrollbar {display:none}

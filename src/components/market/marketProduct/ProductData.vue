        ...
        <div class="productData_main">
            ...
            <div class="content">
                <div class="content_header">
                    <div class="content_header_title">当前所有淘货数量：{{productTotal>1000?(productTotal/1000).toFixed(2)+'k':productTotal}}</div>
                    <div class="content_header_list">
                        <p>汽车总量： {{carTotal.goods_total>1000?(carTotal.goods_total/1000).toFixed(2)+'k':carTotal.goods_total}}</p>
                        <p>当日增加： {{carTotal.goods_total_increase>1000?(carTotal.goods_total_increase/1000).toFixed(2)+'k':carTotal.goods_total_increase}}</p>
                    </div>

                    <div class="content_header_list">
                        <p>摩托车总量： {{motorTotal.goods_total>1000?(motorTotal.goods_total/1000).toFixed(2)+'k':motorTotal.goods_total}}</p>
                        <p>当日增加： {{motorTotal.goods_total_increase>1000?(motorTotal.goods_total_increase/1000).toFixed(2)+'k':motorTotal.goods_total_increase}}</p>
                    </div>

                    <div class="content_header_list">
                        <p>无人机总量： {{planeTotal.goods_total>1000?(planeTotal.goods_total/1000).toFixed(2)+'k':planeTotal.goods_total}}</p>
                        <p>当日增加： {{planeTotal.goods_total_increase>1000?(planeTotal.goods_total_increase/1000).toFixed(2)+'k':planeTotal.goods_total_increase}}</p>
                    </div>

                    <div class="content_header_list">
                        <p>智能设备总量： {{intelligentTotal.goods_total>1000?(intelligentTotal.goods_total/1000).toFixed(2)+'k':intelligentTotal.goods_total}}</p>
                        <p>当日增加： {{intelligentTotal.goods_total_increase>1000?(intelligentTotal.goods_total_increase/1000).toFixed(2)+'k':intelligentTotal.goods_total_increase}}</p>
                    </div>
                </div>
    ...
    export default {
                ...
                breadData: [
                    {
                        ...
                        id: 3,
                        name: '商品数据统计',
                        urls: '/index/market/marketProduct/ProductData',
                        icon: 'icon-home'
                    }
                ],
        ...
        methods: {
                ...
                dates.map(date=> {
                    ...
                    this.carByDate.map(item => {
                        ...
                        if (new Date(year + '-' + month + '-' + date).getTime() === new Date(item.created_at.split(' ')[0]).getTime() ||
                            ...
                            carNum += Number(item.goods_total);
                        }
                    });
                    this.motorByDate.map(item => {
                        if (new Date(year + '-' + month + '-' + date).getTime() === new Date(item.created_at.split(' ')[0]).getTime() ||
                            new Date(year + '-' + (month + 1) + '-' + date).getTime() === new Date(item.created_at.split(' ')[0]).getTime()) {
                            motorNum += Number(item.goods_total);
                        }
                    });
                    this.planeByDate.map(item => {
                        if (new Date(year + '-' + month + '-' + date).getTime() === new Date(item.created_at.split(' ')[0]).getTime() ||
                            new Date(year + '-' + (month + 1) + '-' + date).getTime() === new Date(item.created_at.split(' ')[0]).getTime()) {
                            planeNum += Number(item.goods_total);
                        }
                    });
                    this.intelligentByDate.map(item => {
                        if (new Date(year + '-' + month + '-' + date).getTime() === new Date(item.created_at.split(' ')[0]).getTime() ||
                            new Date(year + '-' + (month + 1) + '-' + date).getTime() === new Date(item.created_at.split(' ')[0]).getTime()) {
                            intelligentNum += Number(item.goods_total);
                        }
                    });
                    car.push(carNum);
            ...
            getProductDataByDate(){
                this.HttpClient.post('/admin/marketAnalysis/oneField',{field:'goods_total',start_time:this.startTime,end_time:this.endTime})
                    .then(res=>{
                        console.log(res);
                        if(res.data.code===200){
                            ...
                            this.drawLine()
                            //计算选择时间区间内的各板块的增量
                            this.carGrowth = this.carByDate[this.carByDate.length-1].goods_total - this.carByDate[0].goods_total;
                            this.motoGrowth = this.motorByDate[this.motorByDate.length-1].goods_total - this.motorByDate[0].goods_total;
                            this.UAVGrowth = this.planeByDate[this.planeByDate.length-1].goods_total - this.planeByDate[0].goods_total;
                            this.intelligenceGrowth = this.intelligentByDate[this.intelligentByDate.length-1].goods_total - this.intelligentByDate[0].goods_total;
            ...
            //获取商品总量
            getProductGross(){
                this.HttpClient.post('/admin/marketAnalysis/getCache',{type:'goods_total'})
                    .then(res=>{
                        console.log(res);
                        if(res.data.code===200){
                            res.data.data.map(item=>{
                                this.productTotal+=Number(item.goods_total);
                                if(item.folder==='汽车'){
                                    this.carTotal=item
                                }else if(item.folder==='摩托'){
...
<style lang="less">
    .productData{
      ...
      .productData_main::-webkit-scrollbar {display:none}
        .productData_main {
            margin-left: 10px;
            margin-top: 10px;
            background: #fff;

...
export default {
  ...
  data() {
    return {
      ...
      articleNum: [], //问题类型数组
      articleTotal: 0, //问题总数
      articleviewTotal:0,//问题浏览量总数
      articleDate: [], //问题数据统计
      articleviewDate:[],//问题浏览量
      car: { normal: 0, business: 0 }, //汽车问题统计
      motorcycle: { normal: 0, business: 0 }, //摩托问题统计
      plane: { normal: 0, business: 0 }, //无人机问题统计
      intelligent: { normal: 0, business: 0 }, //智能设备问题统计
      // 浏览量
      carview: { normal: 0, business: 0 }, //汽车问题统计
      motorcycleview: { normal: 0, business: 0 }, //摩托问题统计
      planeview: { normal: 0, business: 0 }, //无人机问题统计
      intelligentview: { normal: 0, business: 0 }, //智能设备问题统计
      totalByDate: 0, //某时间段内问题总数
      totalByDateview:0,//某段时间内问题浏览量总数
      carArticle: [], //汽车问题统计
      motorArticle: [], //摩托问题统计
      planeArticle: [], //无人机问题统计
      intelligentArticle: [], //智能设备问题统计
      // 浏览量
      carviewArticle: [], //汽车问题统计
      motorviewArticle: [], //摩托问题统计
      planeviewArticle: [], //无人机问题统计
      intelligentviewArticle: [], //智能设备问题统计
      ...
      dates.map(date => {
        ...
        this.carArticle.map(item => {
          if (
            ...
            carNum += Number(item.questions);
            //按用户类型搜索
            if (item.user_type === 1) {
              carNum1 += Number(item.questions);
            } else if (item.user_type === 2) {
              carNum2 += Number(item.questions);
            } else if (item.user_type === 3) {
              carNum3 += Number(item.questions);
            }
          }
        });
        this.motorArticle.map(item => {
          if (
            ...
            motorNum += Number(item.questions);
            //按用户类型搜索
            if (item.user_type === 1) {
              motorNum1 += Number(item.questions);
            } else if (item.user_type === 2) {
              motorNum2 += Number(item.questions);
            } else if (item.user_type === 3) {
              motorNum3 += Number(item.questions);
            }
          }
        });
        this.planeArticle.map(item => {
          if (
            ...
            planeNum += Number(item.questions);
            //按用户类型搜索
            if (item.user_type === 1) {
              planeNum1 += Number(item.questions);
            } else if (item.user_type === 2) {
              planeNum2 += Number(item.questions);
            } else if (item.user_type === 3) {
              planeNum3 += Number(item.questions);
            }
          }
        });
        this.intelligentArticle.map(item => {
          if (
            ...
            intelligentNum += Number(item.questions);
            //按用户类型搜索
            if (item.user_type === 1) {
              intelligentNum1 += Number(item.questions);
            } else if (item.user_type === 2) {
              intelligentNum2 += Number(item.questions);
            } else if (item.user_type === 3) {
              intelligentNum3 += Number(item.questions);
            }
          }
        });
        if (this.userType === 1) {
        ...
        if (res.data.code === 200) {
          ...
          this.articleDate.map(item => {
            if (item.folder_type === this.carMenuID) {
              ...
              if (item.user_type === 1) {
                this.car.normal += Number(item.questions);
                this.totalByDate += Number(item.questions);
              } else {
                this.car.business += Number(item.questions);
                this.totalByDate += Number(item.questions);
              }
            } else if (item.folder_type === this.motorMenuID) {
              this.motorArticle.push(item);
              if (item.user_type === 1) {
                this.motorcycle.normal += Number(item.questions);
                this.totalByDate += Number(item.questions);
              } else {
                this.motorcycle.business += Number(item.questions);
                this.totalByDate += Number(item.questions);
              }
            } else if (item.folder_type === this.planeMenuID) {
              this.planeArticle.push(item);
              if (item.user_type === 1) {
                this.plane.normal += Number(item.questions);
                this.totalByDate += Number(item.questions);
              } else {
                this.plane.business += Number(item.questions);
                this.totalByDate += Number(item.questions);
              }
            } else if (item.folder_type === this.intelligentMenuID) {
              this.intelligentArticle.push(item);
              if (item.user_type === 1) {
                this.intelligent.normal += Number(item.questions);
                this.totalByDate += Number(item.questions);
              } else {
                this.intelligent.business += Number(item.questions);
                this.totalByDate += Number(item.questions);
              }
            }
          });
        ...
        if (res.data.code === 200) {
          ...
          this.articleviewDate.map(item => {
            if (item.folder_type === this.carMenuID) {
              ...
              if (item.user_type === 1) {
                this.carview.normal += Number(item.questions_view);
                this.totalByDateview +=Number(item.questions_view)
              } else {
                this.carview.business += Number(item.questions_view);
                this.totalByDateview +=Number(item.questions_view)
              }
            } else if (item.folder_type === this.motorMenuID) {
              this.motorviewArticle.push(item);
              if (item.user_type === 1) {
                this.motorcycleview.normal += Number(item.questions_view);
                this.totalByDateview +=Number(item.questions_view)
              } else {
                this.motorcycleview.business += Number(item.questions_view);
                this.totalByDateview +=Number(item.questions_view)
              }
            } else if (item.folder_type === this.planeMenuID) {
              this.planeviewArticle.push(item);
              if (item.user_type === 1) {
                this.planeview.normal += Number(item.questions_view);
                this.totalByDateview +=Number(item.questions_view)
              } else {
                this.planeview.business += Number(item.questions_view);
                this.totalByDateview +=Number(item.questions_view)
              }
            } else if (item.folder_type === this.intelligentMenuID) {
              this.intelligentviewArticle.push(item);
              if (item.user_type === 1) {
                this.intelligentview.normal += Number(item.questions_view);
                this.totalByDateview +=Number(item.questions_view)
              } else {
                this.intelligentview.business += Number(item.questions_view);
                this.totalByDateview +=Number(item.questions_view)
              }
            }
          });
      ...
      dates.map(date => {
        ...
        this.carviewArticle.map(item => {
          if (
            ...
            // console.log(item)
            carNumview += Number(item.questions_view);
            //按用户类型搜索
            if (item.user_type === 1) {
              carNumview1 += Number(item.questions_view);
            } else if (item.user_type === 2) {
              carNumview2 += Number(item.questions_view);
            } else if (item.user_type === 3) {
              carNumview3 += Number(item.questions_view);
            }
          }
        });
        this.motorviewArticle.map(item => {
          if (
            ...
            motorNumview += Number(item.questions_view);
            //按用户类型搜索
            if (item.user_type === 1) {
              motorNumview1 += Number(item.questions_view);
            } else if (item.user_type === 2) {
              motorNumview2 += Number(item.questions_view);
            } else if (item.user_type === 3) {
              motorNumview3 += Number(item.questions_view);
            }
          }
        });
        this.planeviewArticle.map(item => {
          if (
            ...
            planeNumview += Number(item.questions_view);
            //按用户类型搜索
            if (item.user_type === 1) {
              planeNumview1 += Number(item.questions_view);
            } else if (item.user_type === 2) {
              planeNumview2 += Number(item.questions_view);
            } else if (item.user_type === 3) {
              planeNumview3 += Number(item.questions_view);
            }
          }
        });
        this.intelligentviewArticle.map(item => {
          if (
            ...
            intelligentNumview += Number(item.questions_view);
            //按用户类型搜索
            if (item.user_type === 1) {
              intelligentNumview1 += Number(item.questions_view);
            } else if (item.user_type === 2) {
              intelligentNumview2 += Number(item.questions_view);
            } else if (item.user_type === 3) {
              intelligentNumview3 += Number(item.questions_view);
            }
          }
        });
  ...
  created() {
    ...
    this.HttpClient.post("/admin/analysis/numCache", { type: 2 }).then(res => {
      // console.log(res);
      if (res.data.code === 200) {
        ...
        this.articleNum.map(item => {
          ...
          item.number = 0;
          item.number += Number(item.analysis.company_questions);
          item.number += Number(item.analysis.normal_questions);
          item.numberview = 0;
          item.numberview += Number(item.analysis.company_questions_view);
          item.numberview += Number(item.analysis.normal_questions_view);

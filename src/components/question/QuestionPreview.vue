<template>
  <div class="questionModify">
    <div class="questionModify_header">{{title}} - 预览</div>
      ...
      <div class="questionModify_List">
        ...
        <div class="list_input list_tags">
          <div class="tags" v-for="(item,index) in articleData.label" :key="index">
            <!-- <div >{{item.name}}</div> -->
            <el-button type="primary" plain disabled>{{item.name}}</el-button>
          </div>
        </div>
      </div>
    ...
    <div class="questionModify_btm">
      <el-button type="primary" @click="handleGoBack">返 回</el-button>
    </div>
  </div>
</template>
